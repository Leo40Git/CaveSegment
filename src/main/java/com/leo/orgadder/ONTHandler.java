...
public class ONTHandler {
	...
	public static final int ONT_RVA = 0x4981E8;
	public static final int[] ONT_REFS = { 0x420F17, 0x420F60 };
	
	public static boolean readOrSetup(PEFile peData) {
		return readONTSection(peData);
	}
	
	public static byte[] write(PEFile peData) {
		int ontNewRVA = writeONTSection(peData);
		ByteBuffer buf = ByteBuffer.allocate(4);
		buf.order(ByteOrder.LITTLE_ENDIAN);
		buf.putInt(0, ontNewRVA);
		for (int ref : ONT_REFS)
			patch(peData, buf, ref);
		byte[] ret = new byte[0];
		try {
			ret = peData.write();
		} catch (IOException e) {
			Main.LOGGER.error("Error while writing executable data", e);
		}
		return ret;
	}
	...
	public static Vector<String> getOrgNames() {
		return orgNames;
	}
	...
	private static int writeONTSection(PEFile peData) {
	...
	private static void setupONTSection(PEFile peData) {
		...
		while (true) {
			...
			int index = 0;
			while (true) {
				ByteBuffer cBuf = read(peData, orgAddr++, 1);
				byte c = cBuf.get(0);
				if (c == 0) {
					// NAME LENGTH RULES:
	...
	private static boolean readONTSection(PEFile peData) {
		int ontSecID = peData.getSectionIndexByTag(".ont");
		if (ontSecID == -1) {
	...
	private static void removeFillerSections(PEFile peData) {
	...
	private static void mallocFiller(PEFile peData, int num, int addr, int size) {
	...
	private static void fixVirtualLayoutGaps(PEFile peData) {
		...
		for (PEFile.Section s : sectionsSorted) {
			...
			if (lastAddress != 0) {
				if (s.virtualAddrRelative != lastAddress) {
					...
					mallocFiller(peData, flrNum++, lastAddress, s.virtualAddrRelative - lastAddress);
				}
			}
			lastSeg = curSeg;
			lastAddress = PEFile.alignForward(s.virtualAddrRelative + s.virtualSize, sectionAlignment);
